/*
 ************************************************************* Nome : DestroyController.java Autor : brunopedroso Data : 31/10/2017
 */
...
public class DestroyController {
  ...
  public ResponseEntity<String> destroyMemory(@PathVariable int power) {
    if (power <= 0)
      power = 1000;
    ...
    return new ResponseEntity<String>("#" + memoryCount++ + ", Total: " + total + ", Used: " + used + ", Free: " + free,
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<String> destroyCpu(@PathVariable int power) {
    if (power <= 0) {
      power = 10;
    }
    ...
    StringBuilder info = new StringBuilder().append("points: ").append(POINTS).append("\n points_per_thread: ")
        .append(points_per_thread).append("\n number_of_executions: ").append(power);
    ...
    for (int i = 0; i < power; i++) {
      ...
      info.append("\n\n pi: ").append(result.toPlainString()).append("\n terror: ").append(error).append("\n in:")
          .append((System.currentTimeMillis() - now.getTime()));
    }
